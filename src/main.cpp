...
int main(int argc, char *argv[])
{
 ...
 QCommandLineOption nosubdirsOption("nosubdirs", "Do not include input folder subdirectories when scraping.");
  ...
  QCommandLineOption includeFilesOption("includefiles", "Tells Skyscraper to only include the files matching the provided asterisk pattern. Remember to double-quote the pattern to avoid weird behaviour. You can add several patterns by separating them with ','. In cases where you need to match for a comma you need to escape it as '\\,'. (Pattern example: '\"Super*\"')", "PATTERN", "");

...
* Made it so the width if the spine of the gamebox can be set to fit the chosen artwork resource so it fits with the height of it aswell taking up all the space of the spine (Thank you to dorkvader for suggesting this)
* Added '--checksum' cli option that allows user to overwrite the sha1 or md5 checksum of a single rom. Will disable if more than one rom is scraped (Thank you to AnalogHero for suggesting this)

...
### Sections
The Skyscraper `config.ini` configuration file is divided into sections.

#### The [main] section
This sections contains all of the general / main configurations for Skyscraper. Options set in this section will be applied overall when running Skyscraper. This is useful if you have your roms on a USB drive or want the localdb resource cache to reside on an external drive. All available options for this section are listed in the `~/.skyscraper/config.ini.example` file. Look for the `[main]` section. A thorough description of all the individual options can be found below.

## Work in progress... The rest of this document is just a copy of the CLI document. Stay tuned...
